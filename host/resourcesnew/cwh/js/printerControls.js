(function() {
	...
	cwhApp.controller("PrinterControlsController", ['$scope', '$http', '$location', '$routeParams', 'cwhWebSocket', 'photonicUtils', function ($scope, $http, $location, $routeParams, cwhWebSocket, photonicUtils) {
		...
		this.cureLayer = function cureLayer() {
			$http.get("services/printers/executeGCode/" + printerName + "/" + "M106%20P5%20S255").then(gCodeSuccess, errorFunction)
			$http.get("services/printers/cureLayer/" + printerName).then(gCodeSuccess, errorFunction)
			$http.get("services/printers/executeGCode/" + printerName + "/" + "M106%20P5%20S200").then(gCodeSuccess, errorFunction)
		}
