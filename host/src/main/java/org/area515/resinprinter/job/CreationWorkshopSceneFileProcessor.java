		...
		try {
			...
			while ((currentLine = stream.readLine()) != null && printer.isPrintActive()) {
				...
				if (matcher.matches()) {
					...
					Process showingSlice = Runtime.getRuntime().exec(new String[]{"nice -n 1 /home/pi/raspidmx/demo_mask_overlay_with_args/show_image", "-d", "5", "-t", Integer.toString(sliceIndex), "-p", printerName, "-e", Integer.toString(sliceExposureDelay), "-b", Integer.toString(numberOfBottomLayers), "-x", Integer.toString(bottomLayerExposureDelay), "-m", "/home/pi/mask/mask.png", slicePath});
